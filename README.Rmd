...
Our algorithm returns a list named ``forest.reg``, which includes two tibbles named ``fitted`` and ``counterfactuals``. ``fitted`` contains individual IDs, optimal treatment arms identified (`trt.rjaf`), predicted optimal outcomes (`Y.rjaf`), and treatment arm clusters (`clus.rjaf`). As counterfactual outcomes present, they are also included in ``fitted`` as `Y.cf`. ``counterfactuals`` contains estimated counterfactual outcomes from every treatment arm. If performing clustering, ``xwalk`` is included, which contains cluster number of treatment assigned by k-means algorithm.
