...
mat growTree_cpp(const vec &y_trainest, const mat &X_trainest,
  ...
  if (reg) { // regularized final outcomes
    ...
    mat_ret.zeros(X_val.n_rows, trts_uniq.n_elem);
    for (unsigned int i = 0; i < type.size(); ++i) { // go thru each terminal node
      ...
      mat_ret.rows(filter_val[i]) += repmat(regavg, filter_val[i].n_elem, 1);
    }
  } else { // unregularized final outcomes
    mat_ret.zeros(X_val.n_rows, X_trainest.n_rows);
    for (unsigned int i = 0; i < type.size(); ++i) {
      mat_ret(filter_val[i], filter_est[i]) += 1.0/filter_est[i].n_elem;
    }
  }
  return mat_ret;
}
...
List growForest_cpp(const vec &y_trainest, const mat &X_trainest,
                    ...
                    const bool &reg=true, const bool &setseed=false,
  ...
  if (reg) {
    for (unsigned int i = 0; i < ntree; ++i) {
      outcome += growTree_cpp(y_trainest, X_trainest, trt_trainest,
                              prob_trainest, X_val, ntrts, nvars, lambda,
                              ipw, nodesize, prop_train, epi, reg);
    }
    outcome /= ntree;
  } else {
    mat mat_wt(X_val.n_rows, X_trainest.n_rows, fill::zeros);
    for (unsigned int i = 0; i < ntree; ++i) {
      mat_wt += growTree_cpp(y_trainest, X_trainest, trt_trainest,
                             prob_trainest, X_val, ntrts, nvars, lambda,
                             ipw, nodesize, prop_train, epi, reg);
    }
    mat_wt /= ntree;
